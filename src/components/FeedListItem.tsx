    ...
    <div className="min-w-0 flex-1">
      <div className="flex items-center gap-2.5 flex-wrap">
        <span className="w-6 h-6 rounded-md flex items-center justify-center shrink-0 bg-white/40 dark:bg-black/20" aria-hidden="true">
          <span className={`text-[11px] font-black ${accentText}`}>{getSourceInitial(feed.name)}</span>
        </span>
        <h4 className={`font-bold truncate text-sm sm:text-base ${accentText}`}>{feed.name}</h4>
      </div>
      <p className={`text-xs truncate mt-1 font-mono opacity-70 ${accentText}`}>{feed.url}</p>
    </div>
  );
}
...
function TestResultBadge({ result }: TestResultBadgeProps) {
  ...
  return (
    <div className={`text-xs font-medium rounded-lg px-3 py-2 flex items-center gap-2 ${bgClass}`}>
      {result.validRss ? (
