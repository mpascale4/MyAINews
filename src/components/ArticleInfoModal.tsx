...
function ArticleDetails({ article }: ArticleDetailsProps) {
  return (
    <div className="space-y-3 text-sm">
      <div>
        <span className="text-sm font-semibold uppercase text-slate-400 tracking-wider">Titolo Notizia</span>
      ...
      <div className="grid grid-cols-2 gap-3 bg-slate-50 dark:bg-slate-800/60 p-3 rounded-2xl border border-slate-100 dark:border-slate-800">
        <div>
          <span className="text-sm text-slate-500 dark:text-slate-400 block">Sorgente</span>
          <p className="font-bold text-slate-900 dark:text-slate-100 truncate">{article.source || "RSS Feed"}</p>
        </div>
        <div>
          <span className="text-sm text-slate-500 dark:text-slate-400 block">Rilevanza AI</span>
      ...
      {article.aiTags && article.aiTags.length > 0 && (
        <div>
          <span className="text-sm font-semibold text-slate-400 block mb-1.5">Tag Tematici:</span>
          <div className="flex flex-wrap gap-1.5">
            {article.aiTags.map((tag, idx) => (
              <span
                key={idx}
                className="px-2.5 py-1 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 rounded-lg text-sm font-medium border border-indigo-100 dark:border-indigo-900"
              >
                #{tag}
              </span>
            ))}
...
function ExcludeSourceButton({ source, onExcludeSource }: ExcludeSourceButtonProps) {
  ...
  return (
    <button
      ...
      className="w-full inline-flex items-center justify-center gap-1.5 px-4 py-2.5 bg-rose-100 hover:bg-rose-200 dark:bg-rose-950/60 dark:hover:bg-rose-900 text-rose-700 dark:text-rose-300 rounded-2xl text-sm font-bold transition-colors cursor-pointer border border-rose-200 dark:border-rose-800"
    >
